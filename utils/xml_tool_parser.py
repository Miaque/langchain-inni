...
class XMLToolParser:
        ...
        for fc_content in function_calls_matches:
            ...
            for function_name, invoke_content in invoke_matches:
                try:
                    ...
                    logger.exception(f"Error parsing invoke block for {function_name}: ", exc_info=e)

        return tool_calls

    def _parse_invoke_block(self, function_name: str, invoke_content: str, full_block: str) -> Optional[XMLToolCall]:
        """Parse a single invoke block into an XMLToolCall."""
        parameters = {}
        parsing_details = {"function_name": function_name, "raw_parameters": {}}
    ...
    def format_tool_call(self, function_name: str, parameters: dict[str, Any]) -> str:
        ...
        for param_name, param_value in parameters.items():
            ...
            lines.append('<parameter name="{}">{}</parameter>'.format(param_name, value_str))

        lines.extend(["</invoke>", "</function_calls>"])
        return "\n".join(lines)

    def validate_tool_call(
        self, tool_call: XMLToolCall, expected_params: Optional[dict[str, type]] = None
