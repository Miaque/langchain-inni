...
class SandboxUploadFileTool(SandboxToolsBase):
    ...
    async def upload_file(
        self, file_path: str, bucket_name: str = "file-uploads", custom_filename: Optional[str] = None
    ) -> ToolResult:
        try:
            ...
            try:
                ...
                if file_info.size > 50 * 1024 * 1024:  # 50MB limit
                    return self.fail_response(
                        f"File '{file_path}' is too large (>50MB). Please reduce file size before uploading."
                    )
            except Exception:
    ...
    async def _get_current_account_id(self) -> str:
        try:
            ...
            thread_result = (
                await client.table("threads").select("account_id").eq("thread_id", thread_id).limit(1).execute()
            )
    ...
    async def _track_upload(
        ...
        try:
            ...
            upload_data = {
                ...
                "metadata": {"uploaded_from": "sandbox", "tool": "upload_file", "secure_upload": True},
            }
