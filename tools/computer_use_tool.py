...
class ComputerUseTool(SandboxToolsBase):
    ...
    @usage_example("""
        ...
        """)
    async def click(
        self, x: Optional[float] = None, y: Optional[float] = None, button: str = "left", num_clicks: int = 1
    ) -> ToolResult:
        ...
        try:
            ...
            result = await self._api_request(
                ...
                {"x": x_int, "y": y_int, "clicks": num_clicks, "button": button.lower()},
            )
            ...
            if result.get("success", False):
                ...
                self.mouse_y = y_int
                return ToolResult(
                    success=True, output=f"{num_clicks} {button} click(s) performed at ({x_int}, {y_int})"
                )
            else:
    ...
    async def scroll(self, amount: int) -> ToolResult:
        ...
        try:
            ...
            result = await self._api_request(
                "POST", "/automation/mouse/scroll", {"clicks": amount, "x": self.mouse_x, "y": self.mouse_y}
            )
    ...
    async def typing(self, text: str) -> ToolResult:
        ...
        try:
            ...
            result = await self._api_request("POST", "/automation/keyboard/write", {"message": text, "interval": 0.01})
    ...
    @openapi_schema(
        {
            ...
            "function": {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {"key": {"type": "string", "description": "Key to press", "enum": KEYBOARD_KEYS}},
                    "required": ["key"],
                },
            },
        }
    ...
    async def mouse_down(
        ...
        try:
            ...
            result = await self._api_request(
                "POST", "/automation/mouse/down", {"x": x_int, "y": y_int, "button": button.lower()}
            )
            ...
            if result.get("success", False):
                ...
                return ToolResult(
                    success=False, output=f"Failed to press button: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
    ...
    async def mouse_up(self, button: str = "left", x: Optional[float] = None, y: Optional[float] = None) -> ToolResult:
        ...
        try:
            ...
            result = await self._api_request(
                "POST", "/automation/mouse/up", {"x": x_int, "y": y_int, "button": button.lower()}
            )
            ...
            if result.get("success", False):
                ...
                return ToolResult(
                    success=False, output=f"Failed to release button: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
    ...
    async def drag_to(self, x: float, y: float) -> ToolResult:
        ...
        try:
            ...
            result = await self._api_request(
                "POST", "/automation/mouse/drag", {"x": target_x, "y": target_y, "duration": 0.3, "button": "left"}
            )
            ...
            if result.get("success", False):
                ...
                return ToolResult(
                    success=True, output=f"Dragged from ({start_x}, {start_y}) to ({target_x}, {target_y})"
                )
            else:
    ...
    async def hotkey(self, keys: str) -> ToolResult:
        ...
        try:
            ...
            result = await self._api_request(
                "POST", "/automation/keyboard/hotkey", {"keys": key_sequence, "interval": 0.01}
            )
