            ...
            try:
                ...
                if not sandbox_info.get("id"):
                    ...
                    try:
                        ...
                        vnc_url = (
                            vnc_link.url if hasattr(vnc_link, "url") else str(vnc_link).split("url='")[1].split("'")[0]
                        )
                        website_url = (
                            website_link.url
                    ...
                    if not update_result.data:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to delete sandbox {sandbox_id} after DB update failure", exc_info=True
                            )
                ...
                logger.error(
                    f"Error retrieving/creating sandbox for project {self.project_id}: {str(e)}", exc_info=True
                )
                raise e
